          ...
          <div class="bg-white rounded-7xl cursor-pointer rounded-b-3xl overflow-hidden shadow-lg transition duration-300 transform hover:shadow-xl">
            ...
            <div class="px-4 py-4 h-60 flex flex-col justify-between">
              <div>
                ...
                <p class="text-gray-600 text-left overflow-hidden overflow-ellipsis whitespace-nowrap">
                  <span v-if="accommodation.distances_and_times_to_campuses">
                    Distance to Mafikeng: {{ accommodation.distances_and_times_to_campuses.Mafikeng.driving.distance }} | Time: {{ accommodation.distances_and_times_to_campuses.Mafikeng.driving.duration }}<br>
  ...
  methods: {
    async fetchAccommodations() {
      try {
        const response = await fetch(
          "http://127.0.0.1:8000/api/accommodations/?university=NWU"
        );
    ...
    async calculateDistancesAndTimes() {
      for (let i = 0; i < this.accommodations.length; i++) {
        ...
        const response = await fetch(
          `http://127.0.0.1:8000/api/distance/?address=${encodeURIComponent(accommodation.street_address)}`
        );
        const data = await response.json();
        this.$set(this.accommodations, i, {
          ...accommodation,
          distances_and_times_to_campuses: data.distances_and_times_to_campuses
        });
      }
    },
